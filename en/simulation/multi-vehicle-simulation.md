...
* for each vehicle,
  ...
  * runs a mavros node
  ...
  ```
  * Note: The complete block for each vehicle is enclosed in a set of `<group>` tags to seperate the ROS namespaces of the vehicles.

To add a third iris to this simulation there are two main compenents to consider:
...
* create a startup file, and change the file as follows:
  ...
  * the second `mavlink start` ports need to match those used in the launch file `fcu_url` arg (Note: Be aware of which port is src and dst for the different endpoints.)

Additional resources:
* See [Simulation](../simulation/README.md) for a discription of the UDP port configuraion.
