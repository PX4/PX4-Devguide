...
The format is self-describing, i.e. it contains the format and message types that are logged (note that the [system logger](../log/logging.md) does allow the *default set* of logged topics in `Logger::add_default_topics()` to be replaced from an SD card).
