...
`./Tools/mavlink_shell.py /dev/ttyACM0` (in the Firmware source). Use ./Tools/mavlink_shell.py -h to get a description of all available arguments which also displays the IP address of wifi connection. For e.g ./Tools/mavlink_shell.py <IP address> can be used to start nsh shell via wifi connection to the autopilot. You may first have to install the
dependencies with `sudo pip install pymavlink pyserial`. You can also start nsh shell on QGC directly: **Analyze -> Mavlink Console**
