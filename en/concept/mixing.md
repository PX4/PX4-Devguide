...
For example the multirotor mixer generates 4, 6, or 8 outputs depending on the geometry, while the summing mixer and null mixer generate just one output.
...
Some mixer definitions have additional tags like `O` and `S` (these are associated with the preceding lines).
