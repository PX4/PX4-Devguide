...
> **Info** This parts cover the sending side for standard FPV 5.8GHz analog FM video. You need to have a compatible receiver and display device to actually consume the live video stream.
...
> **Tip** If you are also including the FPV parts don't forget to hook up
...
I mounted the GPS module using the listed generic multicopter GPS mast because mounting it any closer to the main body made the magnetometer readings totally unusable. An experiment mounting the module to the far back of directly the top of the frame showed 6 times magnetometer magnitude noise likely caused by the magnetic field of the ESC current. Note that I shortened the mast by ~2cm to make it fit better the cable length and the frame dimensions. The GPS module is sticked with double sided tape to top plate of the mast.
...
For this build I pulled the newest PX4 master because it supports the "FMU as task" improvements is explained just below and flashed it to the Pixracer. I used [QGC](http://qgroundcontrol.com/) daily build configure the following:
...
MC_YAWRATE_P = 0.1
```
Note that this gains will likely not work well for you but I wanted to give them just as a reference.

After you found a good tuning for the rate controller, flying in manual mode with the default attitude controller gains is most of the time already ok and tuning it further isn't very hard anymore.
