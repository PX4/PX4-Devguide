...
- [광학 추적(Optical Flow)](#optical_flow) 기술로 2차원 평면상의 속도를 추정합니다(아래 방향으로 향한 카메라와 아래 방향으로 향한 거리 센서 활용).
- [움직임 촬영(Motion Capture)](#mocap) 기술을 통해 이동 장비 *외부*의 비전 시스템으로 3차원 자세를 추정합니다. 실내 공간 탐색에 주로 활용합니다.
...
움직임 촬영(Motion Capture, a.k.a MoCap)은 비행체 *외부*의 위치 결정 방법으로 3차원 *자세*(위치와 방향) 를 추정하는 기술입니다. MoCap systems most commonly detect motion using infrared cameras, but other types of cameras, Lidar, or Ultra Wideband (UWB) may also be used.
